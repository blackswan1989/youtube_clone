...
var path = require("path"); //path는 NodeJS와 함께 기본적으로 설치되는 유틸리티이다.
...
var OUTPUT_DIR = path.join(__dirname, "static"); //Entry(entry_file)와 Output(output_directory)의 차이점은
...
var config = {
  ...
  module: {
    rules: [{
      ...
      test: /\.(scss)$/,
      use: ExtractCSS.extract([//4)순수한css가 불러와지면 그 부분만 추출해서 보내준다.
      {
        loader: "css-loader" //3)webpack이 css를 이해할 수 있게 해준다.

      }, {
        loader: "postcss-loader",
        //2)css를 받아서 plugin을 가지고 css를 호환성있게 변환.
        options: {
          postcssOptions: {
...
module.exports = config; // module { rules:[ ] } 코드 설명
